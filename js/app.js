...
async function initializeApp() {
    ...
    setTimeout(async () => {
        ...
        if (typeof SessionManager !== 'undefined') {
            const sessionRestored = await SessionManager.initializeSession();
            
            if (sessionRestored) {
                console.log('✅ App initialized with restored session');
                if (typeof window.showAlert === 'function') {
                    window.showAlert('Oturumunuz devam ediyor...', 'success');
                }
                return;
            }
        }
...
function enhanceLoginForm() {
    ...
    setTimeout(() => {
        const loginForm = document.querySelector('#loginScreen .login-container');
        if (loginForm && !document.getElementById('rememberMe')) {
            // Add Remember Me checkbox after password field
            ...
            if (passwordGroup) {
                ...
                rememberMeDiv.innerHTML = `
                    <div style="display: flex; align-items: center; margin-top: 10px;">
                        ...
                        <label for="rememberMe" style="color: #666; margin: 0;">Beni Hatırla</label>
                    </div>
                `;
                passwordGroup.parentNode.insertBefore(rememberMeDiv, passwordGroup.nextSibling);
            }
        }
    }, 500);
}
