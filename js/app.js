...
// Defensive populate functions for Shipping, Stock and Reports.
// Paste these into app.js immediately AFTER the switchTab(...) function.

function $id(id) { return document.getElementById(id); }

async function populateShippingTable() {
    try {
        console.log('populateShippingTable: start');
        const tbody = $id('shippingTableBody');
        if (!tbody) {
            console.warn('populateShippingTable: #shippingTableBody not found in DOM');
            return;
        }
        tbody.innerHTML = '<tr><td colspan="6">Yükleniyor...</td></tr>';

        let rows = Array.isArray(window.containers) ? window.containers.slice() : [];
        ...
        if (typeof supabase !== 'undefined' && supabase && navigator.onLine) {
            try {
                ...
                console.warn('populateShippingTable: supabase fetch failed', e);
            }
        }

        if (!rows || rows.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; color:#666;">Kayıt bulunamadı</td></tr>';
            return;
        }

        tbody.innerHTML = rows.map(row => {
            const id = escapeHtml(row.id || row.container_no || '');
            const no = escapeHtml(row.container_no || '');
            const status = escapeHtml(row.status || '-');
            const packagesCount = escapeHtml(row.package_count != null ? row.package_count : (row.packages ? row.packages.length : '-'));
            const totalQty = escapeHtml(row.total_quantity != null ? row.total_quantity : '-');
            const created = escapeHtml(new Date(row.created_at || row.createdAt || Date.now()).toLocaleString('tr-TR'));
            return `<tr data-container-id="${id}">
                        <td>${no}</td>
                        <td>${packagesCount}</td>
                        <td>${totalQty}</td>
                        <td>${status}</td>
                        <td>${created}</td>
                        <td><button class="btn btn-sm" onclick="viewContainerDetail('${id}')">Detay</button></td>
                    </tr>`;
        }).join('');
    } catch (error) {
        console.error('populateShippingTable error:', error);
        const tbody = $id('shippingTableBody');
        if (tbody) tbody.innerHTML = `<tr><td colspan="6" style="color:#c00">Hata: ${escapeHtml(error.message || error)}</td></tr>`;
    }
}

async function populateStockTable() {
    try {
        console.log('populateStockTable: start');
        const tbody = $id('stockTableBody');
        if (!tbody) {
            console.warn('populateStockTable: #stockTableBody not found in DOM');
            return;
        }
        tbody.innerHTML = '<tr><td colspan="6">Yükleniyor...</td></tr>';
            ...
            try {
                ...
                if (!error && Array.isArray(data) && data.length) {
                    tbody.innerHTML = data.map(item => {
                        const name = escapeHtml(item.name || item.product_name || '-');
                        const code = escapeHtml(item.code || item.sku || '-');
                        const qty = escapeHtml(item.quantity != null ? item.quantity : '-');
                        const updated = escapeHtml(new Date(item.updated_at || item.updatedAt || Date.now()).toLocaleString('tr-TR'));
                        return `<tr>
                                    <td>${name}</td>
                                    <td>${code}</td>
                ...
                console.warn('populateStockTable: supabase stock fetch failed', e);
            }
        }

        let packagesList = Array.isArray(window.packages) ? window.packages : [];
        if (packagesList.length === 0 && typeof ExcelJS !== 'undefined' && ExcelJS.readFile) {
            try { packagesList = await ExcelJS.readFile(); } catch (e) { console.warn('ExcelJS.readFile failed', e); }
        }

        const aggregate = {};
        packagesList.forEach(pkg => {
            const items = pkg.items || pkg.items_array || {};
            if (Array.isArray(items)) {
                items.forEach(it => {
                    const key = it.name || it.product || 'unknown';
        ...
        if (keys.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; color:#666;">Stok verisi bulunamadı</td></tr>';
            return;
        }

        tbody.innerHTML = keys.map(name => {
            const qty = aggregate[name];
            return `<tr>
        ...
        console.error('populateStockTable error:', error);
        const tbody = $id('stockTableBody');
        if (tbody) tbody.innerHTML = `<tr><td colspan="6" style="color:#c00">Hata: ${escapeHtml(error.message || error)}</td></tr>`;
    }
}
...
async function populateReportsTable() {
    try {
        console.log('populateReportsTable: start');
        const tbody = $id('reportsTableBody');
        if (!tbody) {
            console.warn('populateReportsTable: #reportsTableBody not found in DOM');
            return;
        }
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (key && key.startsWith('report_')) {
                try {
                    ...
                    localReports.push({
                        key,
                        fileName: parsed.fileName || key.replace(/^report_/, ''),
                        date: parsed.date || parsed.created_at || null,
                        packageCount: parsed.packageCount || (parsed.packages ? parsed.packages.length : 0)
                    });
                } catch (e) {
                    localReports.push({ key, fileName: key.replace(/^report_/, ''), date: null, packageCount: 0 });
                }
            }
        }
            ...
            try {
                ...
                if (!error && Array.isArray(data)) {
                    remoteReports = data.map(r => ({
                        key: `supabase_${r.id || r.fileName}`,
                        fileName: r.fileName || (r.id ? `report_${r.id}` : 'report'),
                        date: r.created_at || r.date || null,
                        packageCount: r.packageCount || (r.packages ? r.packages.length : 0)
                    }));
                } else if (error) {
                    console.warn('populateReportsTable: supabase reports error', error);
                }
            } catch (e) {
                console.warn('populateReportsTable: supabase fetch failed', e);
            }
        }

        const all = [...localReports, ...remoteReports];
        if (all.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; color:#666;">Rapor bulunamadı</td></tr>';
            return;
        }

        tbody.innerHTML = all.map(r => {
            const fileName = escapeHtml(r.fileName);
            const date = r.date ? escapeHtml(new Date(r.date).toLocaleString('tr-TR')) : '-';
            const count = escapeHtml(r.packageCount != null ? r.packageCount : '-');
            return `<tr data-report-key="${escapeHtml(r.key)}">
                        <td>${fileName}</td>
                        <td>${date}</td>
                        <td style="text-align:right">${count}</td>
                        <td>
                            <button class="btn btn-sm" onclick="viewReport('${fileName}')">Görüntüle</button>
                            <button class="btn btn-sm" onclick="downloadReport('${fileName}')">İndir</button>
                            <button class="btn btn-sm" onclick="deleteReport('${fileName}')">Sil</button>
                        </td>
                    </tr>`;
        }).join('');
    } catch (error) {
        console.error('populateReportsTable error:', error);
        const tbody = $id('reportsTableBody');
        if (tbody) tbody.innerHTML = `<tr><td colspan="5" style="color:#c00">Hata: ${escapeHtml(error.message || error)}</td></tr>`;
    }
}

// Expose to global scope so switchTab and other scripts can call them
